...
import { User, Mail, Smartphone, Lock, ArrowLeft, UserCircle } from 'lucide-react';
...
export function PatientRegistration({ onBack }: PatientRegistrationProps) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            // Check if logged in (auto-confirm)
            const { data: { session } } = await supabase.auth.getSession();
            if (session) {
                // App.tsx listener will redirect to dashboard
            } else {
        ...
        <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-50 flex items-center justify-center p-4">
            <div className="w-full max-w-md bg-white rounded-2xl shadow-xl p-8">
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <Button
                        type="button"
                        onClick={() => authService.signInWithGoogle('patient')}
                        variant="outline"
