...
=======
  const handleKeyDown = (e: React.KeyboardEvent) => {
    if (e.key === 'Enter') {
      ...
      // Allow default behavior for Textarea to enable new lines
      if (target instanceof HTMLTextAreaElement) return;

      // Allow default for buttons (Select triggers) and checkboxes/files
...
=======
      ...
      if (session) {
        // App.tsx auth listener will handle redirection to dashboard
        console.log("Auto-login detected, waiting for redirect...");
      } else {
...
=======
          <Select onValueChange={(v) => handleSelectChange('establishedYear', v)} value={formData.establishedYear}>
            ...
            <SelectContent>
              {Array.from({ length: new Date().getFullYear() - 1900 + 1 }, (_, i) => (new Date().getFullYear() - i).toString()).map((year) => (
